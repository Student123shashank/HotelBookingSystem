...
const UserBookingHistory = () => {
  ...
  useEffect(() => {
    const fetchBookingHistory = async () => {
      try {
        const response = await axios.get("http://localhost:1000/api/v1/get-booking-history", { headers });
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {bookingHistory.map((item) => (
              <div key={item._id} className="bg-white dark:bg-zinc-800 rounded-lg shadow-md p-4 flex flex-col gap-4">
                ...
                />
                <div>
                  <h1 className="text-lg font-semibold text-zinc-900 dark:text-white">{item.hotel.name}</h1>
                  <p className="text-sm text-zinc-600 dark:text-zinc-400">{item.hotel.description.slice(0, 100)}...</p>
                  <p className="text-sm text-zinc-700 dark:text-zinc-300">Location: {item.hotel.location}</p>
                  <p className="text-lg font-bold text-yellow-500">₹{item.hotel.pricePerNight} per night</p>
                </div>
