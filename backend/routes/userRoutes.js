...
router.get("/total-users", async (req, res) => {
    try {
      ...
      return res.json({ status: "Success", count: totalUsers });
    } catch (error) {
      console.error("Error fetching total users:", error);
      return res.status(500).json({ status: "Error", message: "An error occurred while fetching total users" });
    }
  });
...
router.get("/all-users", async (req, res) => {
  ...
  router.put("/update-user/:id", async (req, res) => {
    try {
      ...
      await User.findByIdAndUpdate(req.params.id, { username, email, address });
